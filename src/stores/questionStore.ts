  ...
  persist(
    (set, get) => ({
      ...
      nextQuestion: () => {
        ...
        if (!currentSession) {
          console.log('nextQuestion: No current session');
          return;
        }

        const nextIndex = currentSession.currentQuestionIndex + 1;
        if (nextIndex >= currentSession.questions.length) {
          console.log('nextQuestion: No more questions available', {
        ...
        console.log('nextQuestion: Moving to next question', {
          fromIndex: currentSession.currentQuestionIndex,
          toIndex: nextIndex,
